    ...
    products.map(async (product) => {
      ...
      if (!item) {
        ...
        const game = await strapi.services.game.create({
          ...
          publishers: await getByName(product.publiser, 'publisher'),
...
module.exports = {
  populate: async (params) => {
    try {
      const gogApuUrl = `https://www.gog.com/games/ajax/filtered?mediaType=game&${qs.stringify(params)}`

      const { data: { products } } = await axios.get(gogApuUrl)
